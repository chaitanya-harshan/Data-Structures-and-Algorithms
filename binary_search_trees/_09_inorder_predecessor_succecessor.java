...
public class _09_inorder_predecessor_succecessor {
    public static List<Integer> predecessorSuccessor(TreeNode root, int key) {
        if (root == null) return new ArrayList<>(Arrays.asList(-1,-1));
        int predecessor = -1;
        int successor = -1;
...
2. The successor of a node in BST is that node that will be visited immediately after the given node 
    in the inorder traversal of the tree. If the given node is visited last in the inorder traversal, 
    then its successor is NULL.

3. The node for which the predecessor and successor will not always be present. If not present, you 
    can hypothetically assume it's position (Given that it is a BST) and accordingly find out the 
